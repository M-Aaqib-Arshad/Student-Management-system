...
export default function Hero() {
  ...
  const getTodos = async () => {
    ...
    const q = query(collection(firestore, "users"), where("roles", "!=", "admin"))
  ...
  const handleDelete = async (user) => {
    ...
    try {
      ...
      let documentsAfterDelete = documents.filter(doc => doc.id !== user.uid)
                  ...
                  <tbody>
                    {documents.map((user, i) => {
                      ...
                      return (
                        <tr key={i}>
                          <th>{i + 1}</th>
                          <td>{user.file ? <Image src={user.file} className='rounded-circle' style={{ width: 50 }} /> : <Avatar size={50} icon={<UserOutlined />} alt={user.name} />}</td>
                          <td>{user.uid}</td>
                          <td>{user.name}</td>
                          <td>{user.address}</td>
                          <td>{user.email}</td>
                          <td>{user.DoB ? dayjs(user.DoB).format("dddd, DD/MM/YYYY") : ""}</td>
                          <td>{user.status}</td>
                          <td>
                            <Space>
                              <Tooltip title="Delete" color='red'><Button danger icon={<DeleteOutlined />} onClick={() => { handleDelete(user) }} /></Tooltip>
                              <Tooltip title="Edit"><Button type="primary" icon={<EditOutlined />} onClick={() => { navigate(`/dashboard/users/${user.uid}`) }} /></Tooltip>
                            </Space>
                          </td>
                        </tr>
                      )
